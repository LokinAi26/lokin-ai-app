...
export default function DriverDispatch() {
  ...
  async function accept(order) {
    ...
    try {
      ...
      await load();
      navigate(`/ai-gps?focus=locked&order=${encodeURIComponent(order.id)}&pickup=${encodeURIComponent(order.pickup_address || "")}&dropoff=${encodeURIComponent(order.dropoff_address || "")}`);
