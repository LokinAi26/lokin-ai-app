...
export default function SecuritySentinel({ compact = false }) {
  ...
  return <section className="rounded-3xl border border-primary/20 bg-gradient-to-b from-primary/[.07] to-black p-5 overflow-hidden relative">
    ...
    <div className="relative mt-2 grid grid-cols-3 gap-2"><Metric icon={ShieldCheck} label="SURFACES" value={String(surfaces.total)}/><Metric icon={ShieldAlert} label="CRITICAL" value={String(surfaces.critical)}/><Metric icon={LockKeyhole} label="POLICY" value="LEAST PRIV"/></div>
