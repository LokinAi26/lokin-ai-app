...
import { getAttackSurfaceSummary } from "@/lib/lokinAttackSurfaceRegistry";
...
export default function SecuritySentinel({ compact = false }) {
  ...
  const surfaces = getAttackSurfaceSummary();
  if (compact) return (
  ...
  return <section className="rounded-3xl border border-primary/20 bg-gradient-to-b from-primary/[.07] to-black p-5 overflow-hidden relative">
    ...
    <div className="relative mt-5 grid grid-cols-2 gap-2"><Metric icon={LockKeyhole} label="SECURITY POSTURE" value={s.posture.toUpperCase()}/><Metric icon={BrainCircuit} label="TRUST SCORE" value={`${s.score}/100`}/></div>
    <div className="relative mt-2 grid grid-cols-2 gap-2"><Metric icon={Radar} label="ADAPTIVE MODE" value={s.adaptiveMode.toUpperCase()}/><Metric icon={Zap} label="INGRESS POLICY" value="FAIL-CLOSED"/></div>
    <div className="relative mt-2 rounded-2xl border border-white/10 bg-black/45 p-3 flex items-center justify-between gap-3"><div><div className="text-[9px] tracking-[.14em] text-white/30">LEAST-PRIVILEGE COVERAGE</div><div className="mt-1 text-xs font-bold text-white">{surfaces.total} modeled surfaces · {surfaces.critical} critical</div></div><div className="rounded-full border border-primary/25 bg-primary/10 px-2.5 py-1 text-[9px] font-bold text-primary">ENFORCED</div></div>
    <div className="relative mt-2 flex items-center justify-between text-[10px] text-white/35 px-1"><span>{s.blocked} blocked</span><span>{s.accepted} validated</span><span>deny by default</span></div>
