...
export default function LockInScore({ score, compact = false }) {
  ...
  const color = overall >= 80 ? "#34d399" : overall >= 55 ? "#facc15" : "#f87171";
  const r = 52;
    ...
    return (
      <div className="flex items-center gap-2">
        ...
        <div className="leading-tight">
          <div className="text-[10px] uppercase tracking-wider text-muted-foreground">Lock In Score</div>
        </div>
      </div>
    );
  }
  ...
  return (
    <div className="rounded-2xl border border-border bg-card p-5">
      <div className="flex items-center gap-2 mb-3">
        ...
        <div className="text-sm font-semibold">Lock In Score</div>
      </div>
        ...
        <div className="relative h-28 w-28 shrink-0">
          <svg viewBox="0 0 120 120" className="h-28 w-28 -rotate-90">
            ...
            <circle cx="60" cy="60" r={r} stroke={color} strokeWidth="10" fill="none"
              strokeLinecap="round" strokeDasharray={`${dash} ${circ}`} style={{ filter: `drop-shadow(0 0 6px ${color})` }} />
          </svg>
          ...
          {CATEGORIES.map((c) => {
            ...
            return (
              <div key={c.key} className="flex items-center gap-2">
                ...
                <span className="text-[10px] font-medium w-6 text-right">{v}</span>
              </div>
            );
          })}
        </div>
